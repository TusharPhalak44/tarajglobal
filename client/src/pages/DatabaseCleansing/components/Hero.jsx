...
const FLOW_STAGES = [
  { id: 'audit',    label: 'CRM Database Ingest & Audit', sub: 'Decay & Duplicate Analysis',     color: '#00A6FF', icon: 'database' },
  { id: 'syntax',   label: 'Syntax & Deduplication',      sub: 'Merging Duplicates & Formats',    color: '#38BDF8', icon: 'dedup'    },
  { id: 'smtp',     label: 'Live SMTP Mailbox Ping',      sub: 'Purging Invalids & Spam Traps',   color: '#00A6FF', icon: 'smtp'     },
  { id: 'turnover', label: 'Turnover & Job Changes',      sub: 'Replacing Departed Executives',   color: '#FFA600', icon: 'refresh'  },
  { id: 'enrich',   label: 'Direct Dial & Tech Append',   sub: 'Appending Missing Cell Numbers',  color: '#72D669', icon: 'enrich'   },
  { id: 'pristine', label: 'Pristine Database Restored',  sub: '99%+ Health Score Back to CRM',   color: '#FF6D00', icon: 'shield'   },
]

const STATUS_PILLS = [
  { label: 'Audit Complete',    dot: '#00A6FF' },
  { label: 'Duplicates Merged', dot: '#38BDF8' },
  { label: 'Inboxes Verified',  dot: '#FFA600' },
  { label: 'Turnover Fixed',    dot: '#72D669' },
  { label: '99%+ Health Score', dot: '#FF6D00' },
]

function DatabaseIcon({ color }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <ellipse cx="9" cy="4.5" rx="7" ry="2.5" stroke={color} strokeWidth="1.5" />
      <path d="M2 4.5v4.5c0 1.38 3.13 2.5 7 2.5s7-1.12 7-2.5V4.5" stroke={color} strokeWidth="1.5" />
      <path d="M2 9v4.5c0 1.38 3.13 2.5 7 2.5s7-1.12 7-2.5V9" stroke={color} strokeWidth="1.5" />
    </svg>
  )
}

function DedupIcon({ color }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <rect x="2" y="2" width="9" height="9" rx="2" stroke={color} strokeWidth="1.5" />
      <rect x="7" y="7" width="9" height="9" rx="2" stroke={color} strokeWidth="1.5" strokeDasharray="2 2" />
    </svg>
  )
}

function SmtpIcon({ color }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      ...
      <circle cx="13" cy="11" r="2.5" fill={color} />
      <path d="M12.5 11l.7.7 1.3-1.4" stroke="#000" strokeWidth="0.8" strokeLinecap="round" />
    </svg>
  )
}
...
function RefreshIcon({ color }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <path d="M15 9A6 6 0 1 1 13 4.5L15 4v4h-4" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}

function EnrichIcon({ color }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <path d="M9.5 2L3.5 10H8.5L8 16L14.5 8H9.5L9.5 2Z" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
...
function ShieldIcon({ color }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <path d="M9 2L3 4.5V9.5C3 13.5 9 16.5 9 16.5C9 16.5 15 13.5 15 9.5V4.5L9 2Z" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M6.5 9.5L8 11L11.5 7.5" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}

function StageIcon({ type, color }) {
  if (type === 'database') return <DatabaseIcon color={color} />
  if (type === 'dedup')    return <DedupIcon color={color} />
  if (type === 'smtp')     return <SmtpIcon color={color} />
  if (type === 'refresh')  return <RefreshIcon color={color} />
  if (type === 'enrich')   return <EnrichIcon color={color} />
  if (type === 'shield')   return <ShieldIcon color={color} />
  return null
}
...
function CommandCenter({ prefersReducedMotion }) {
  ...
  return (
    <div
      ...
      role="img"
      aria-label="Animated CRM Database Cleansing workflow from raw database ingest to pristine verified data"
    >
      {/* Grid background */}
      <div
      ...
      <div className="border-t border-white/[0.05] px-5 py-3 grid grid-cols-3 gap-3">
        {[
          { label: 'Scrub Engine', value: 'Active Live', color: '#72D669' },
          { label: 'Bounce Rate', value: '< 1.2%', color: '#FFA600' },
          { label: 'Health Score', value: '99.4% Ready', color: '#00A6FF' },
        ].map(m => (
...
const Hero = () => {
  ...
  const handleSeeHowItWorks = () => {
    const el = document.getElementById('cleansing-process')
  ...
  return (
    <section
      ...
      aria-label="Database Cleansing Hero"
    >
      {/* Background */}
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-4 sm:pt-6 sm:pb-6 lg:pt-6 lg:pb-6">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="flex flex-col justify-center order-1 lg:order-1">
            ...
            >
              <span className="block pb-1 sm:pb-1.5">Turn Stale Databases</span>
              <span
              ...
              >
                Into Accurate Business Revenue
              </span>
            </motion.h1>
            ...
            >
              Protect your sender reputation and accelerate sales efficiency. We scrub, deduplicate, verify, and enrich your stale CRM databases, reducing bounce rates below 1% and maximizing sales rep connection velocity.
            </motion.p>
