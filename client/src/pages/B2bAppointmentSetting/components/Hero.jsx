...
// ─── Command Center Appointment Setting Flow Visual ──────────────────────────

const FLOW_STAGES = [
  { id: 'target',    label: 'Target Account',       sub: 'Ideal Customer Profile (ICP)',     color: '#00A6FF', icon: 'target'    },
  { id: 'decision',  label: 'Decision Maker',        sub: 'Verified Economic Buyer',          color: '#38BDF8', icon: 'person'    },
  { id: 'outreach',  label: 'Multi-Channel Outreach', sub: 'Phone, Email & Social Sprints',    color: '#00A6FF', icon: 'outreach'  },
  { id: 'engage',    label: 'Buyer Engagement',      sub: 'Conversational Discovery',         color: '#FFA600', icon: 'engage'    },
  { id: 'qualify',   label: 'Strict Qualification',  sub: 'Need, Timing & Authority Fit',     color: '#72D669', icon: 'qualify'   },
  { id: 'booked',    label: 'Booked Sales Meeting',  sub: 'AE Calendar & Briefing Dossier',   color: '#FF6D00', icon: 'booked'    },
]

const STATUS_PILLS = [
  { label: 'Targeted',   dot: '#00A6FF' },
  { label: 'Researched', dot: '#FFA600' },
  { label: 'Engaged',    dot: '#38BDF8' },
  { label: 'Qualified',  dot: '#72D669' },
  { label: 'Booked',     dot: '#FF6D00' },
]
...
function StageIcon({ type, color }) {
  ...
  if (type === 'outreach') return <OutreachIcon color={color} />
  if (type === 'engage')   return <EngageIcon color={color} />
  if (type === 'qualify')  return <QualifyIcon color={color} />
  if (type === 'booked')   return <BookedIcon color={color} />
  return null
}
...
function CommandCenter({ prefersReducedMotion }) {
  ...
  return (
    <div
      ...
      role="img"
      aria-label="Animated B2B appointment setting workflow from target accounts to booked sales meetings"
    >
      {/* Grid background */}
      <div
      ...
      <div className="flex items-center gap-2 px-5 py-3.5 border-b border-white/[0.06]">
        ...
        <span className="ml-2 text-[10px] font-mono font-bold tracking-[0.2em] text-[#00A6FF]/80 uppercase">
          Appointment Setting Command Center
        </span>
      ...
      <div className="border-t border-white/[0.05] px-5 py-3 grid grid-cols-3 gap-3">
        {[
          { label: 'Delivery Status', value: 'Active', color: '#72D669' },
          { label: 'Avg. Show Rate', value: '85%+', color: '#FFA600' },
          { label: 'Meeting SLA', value: 'Guaranteed', color: '#00A6FF' },
        ].map((m) => (
...
const Hero = () => {
  ...
  const handleSeeHowItWorks = () => {
    const el = document.getElementById('appointment-process')
  ...
  return (
    <section
      ...
      aria-label="B2B Appointment Setting Hero"
    >
      {/* Background */}
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-4 sm:pt-6 sm:pb-6 lg:pt-6 lg:pb-6">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="flex flex-col justify-center order-1 lg:order-1">
            ...
            >
              <span className="block pb-1 sm:pb-1.5">Turn Qualified B2B Prospects</span>
              <span
            ...
            >
              Taraj Global helps B2B technology and SaaS companies connect with relevant decision-makers and create qualified sales appointments through targeted prospecting, personalized outreach, and buyer intelligence.
            </motion.p>
