...
const AUDIENCES = [
  {
    ...
    description:
      'Reach relevant technology decision-makers with pre-screened budget fit, identified software pain points, and active purchasing timelines.',
    points: [
      'Reach relevant technology decision-makers',
  ...
  },
  {
    ...
    tagline: 'TARGETED ACCOUNTS',
    color: '#7C3AED',
    image: '/it-audience.jpg',
    description:
      'Connect with targeted accounts and buying stakeholders across complex hardware, cloud infrastructure, and enterprise systems.',
    points: [
      'Connect with targeted accounts and buying stakeholders',
  ...
  },
  {
    id: 'sales-teams',
    num: '03',
    label: 'Growing Sales Teams',
    tagline: 'EXTEND PROSPECTING CAPACITY',
    color: '#72D669',
    image: '/services-audience.jpg',
    description:
      'Extend prospecting capacity and create more conversations so quota-carrying Account Executives can focus 100% on closing deals.',
    points: [
      'Extend prospecting capacity and create more conversations',
      'Steady calendar flow of 10 to 40+ sales-ready discovery calls per month',
      '85%+ meeting hold rate with proactive calendar reminder cadences',
    ],
    visualType: 'services',
  },
  {
    id: 'new-markets',
    num: '04',
    label: 'Companies Entering New Markets',
    tagline: 'NEW SEGMENT PENETRATION',
    color: '#FF6D00',
    image: '/enterprise-audience.jpg',
    description:
      'Build conversations with relevant accounts in new segments and geographies without the risk, overhead, or time delay of hiring internal SDRs.',
    points: [
      'Build conversations with relevant accounts in new segments',
      'Multi-stakeholder buying committee consensus mapping',
      'Accelerated market penetration with localized buyer intelligence',
    ],
...
const AudienceVisualBackground = ({ type, color }) => {
  ...
  if (type === 'services') {
    return (
      <svg
        ...
        fill="none"
      >
        {/* Professional Services & Executive Flow */}
        <polygon points="150,50 250,110 250,210 150,270 50,210 50,110" stroke={color} strokeWidth="1.2" strokeDasharray="4 4" />
        <circle cx="150" cy="160" r="45" stroke={color} strokeWidth="1.5" />
        <path d="M135 155 L145 165 L165 145" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        <line x1="150" y1="50" x2="150" y2="115" stroke={color} strokeWidth="1.2" />
        <line x1="150" y1="205" x2="150" y2="270" stroke={color} strokeWidth="1.2" />
      </svg>
    )
  }
  ...
  return (
    <svg
      ...
      fill="none"
    >
      {/* Enterprise Multi-Stakeholder Matrix */}
      <circle cx="150" cy="70" r="30" stroke={color} strokeWidth="1.5" />
      <circle cx="150" cy="70" r="10" fill={color} />
      <text x="150" y="115" textAnchor="middle" fill={color} fontSize="9" fontFamily="monospace">STEERING</text>

      <path d="M150 100 V150 M150 150 H90 M150 150 H210" stroke={color} strokeWidth="1.5" />
...
const WhoIsItFor = () => {
  ...
  // GSAP ScrollTrigger Pinned Storytelling on Desktop (Identical to master reference page)
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ScrollTrigger.create({
        id: 'who-is-it-for-appointment-scroll',
        trigger: sectionRef.current,
        pin: desktopPinRef.current,
        start: 'center center',
        end: '+=1600px', // scroll distance matching master template
        scrub: 1,
        anticipatePin: 1,
        onUpdate: (self) => {
  ...
  const handleSelect = (idx) => {
    ...
    if (isDesktop && !prefersReducedMotion) {
      const st = ScrollTrigger.getById('who-is-it-for-appointment-scroll')
      if (st) {
  ...
  return (
    <section
      ref={sectionRef}
      id="who-benefits-from-appointment-setting"
      className="relative py-6 sm:py-8 lg:py-8 overflow-hidden bg-background"
      aria-label="Who Is B2B Appointment Setting For?"
    >
      {/* Background accents — clean, soft, with very subtle radial light */}
      ...
      >
        {/* Section header — Centered, matching master template */}
        <div className="max-w-4xl mx-auto text-center mb-3.5 lg:mb-4">
          ...
          >
            Taraj Global connects you with the right decision-makers through targeted prospecting and appointment setting. Focus on qualified conversations, not prospecting.
          </motion.p>
        </div>
