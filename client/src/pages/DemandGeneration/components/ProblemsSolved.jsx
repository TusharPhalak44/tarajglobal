...
// ─── 5 Problem → Engine → Solution Transformation Pairs (Demand Generation) ───
...
const FLOW_ITEMS = [
  {
    ...
    num: '01',
    category: 'TARGETING',
    engineLabel: 'ICP Demand Engine',
    engineAction: 'Precision Targeting',
    problem: {
      title: 'Untargeted Lists',
      sub: 'Broad, Low-Relevance Outreach',
      icon: FilterX,
    },
    solution: {
      title: 'Targeted B2B Audiences',
      sub: 'Verified Decision-Maker Focus',
      icon: Focus,
    },
    color: '#00A6FF', // Blue
  ...
  },
  {
    ...
    engineAction: 'Contextual Engagement',
    problem: {
      title: 'Low Engagement',
      sub: 'Ignored Content & Cold Clicks',
      icon: MailX,
    },
    solution: {
      title: 'Multi-Channel Demand',
      sub: 'High-Value Buyer Interactions',
  ...
  },
  {
    ...
    engineAction: 'Zero-Waste Validation',
    problem: {
      title: 'Poor Quality Leads',
      sub: 'Unqualified Contacts & Bad Data',
      icon: ShieldAlert,
    },
    solution: {
  ...
  },
  {
    ...
    engineAction: 'Full-Funnel Nurture',
    problem: {
      title: 'Inconsistent Cadence',
      sub: 'Single-Touch Stalled Funnels',
      icon: Workflow,
    },
    solution: {
      title: 'Multi-Touch Nurturing',
      sub: 'Predictable Revenue Pipelines',
      icon: Send,
    },
    color: '#F59E0B', // Amber
  ...
  },
  {
    ...
    problem: {
      ...
      sub: 'Unclear Attribution & Blind Spots',
      icon: EyeOff,
    },
    solution: {
...
const ProblemsSolved = () => {
  ...
  return (
    <section
      id="what-problems-b2b-demand-generation-solves"
      ref={sectionRef}
      ...
      aria-label="What Problems Does B2B Demand Generation Solve"
    >
      ...
      <div className="relative max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-4 sm:mb-5">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black tracking-tight leading-tight uppercase">
            <span className="block">What Problems Does</span>
            <span className="block bg-gradient-to-r from-primary via-[#00c8ff] to-cta bg-clip-text text-transparent whitespace-nowrap">
              B2B Demand Generation Solve?
            </span>
          </h2>

          <p className="text-sm sm:text-base text-slate-600 dark:text-slate-400 leading-relaxed max-w-xl mx-auto mt-1.5 font-normal">
            From untargeted lists to low engagement and stalled pipeline, we transform core bottlenecks into sales-ready conversations.
          </p>
        ...
        >
          ...
          >
            <defs>
              <filter id="line-glow-dg" x="-20%" y="-20%" width="140%" height="140%">
              ...
              {FLOW_ITEMS.map((item) => (
                <React.Fragment key={`arrow-dg-${item.id}`}>
                  <marker
                    id={`arrow-head-dg-${item.id}`}
                    viewBox="0 0 10 10"
                    refX="7"
                    refY="5"
                    markerWidth="6"
              ...
              return (
                <g key={`left-group-dg-${i}`}>
                  ...
                  <path
                    ...
                    markerEnd={`url(#${isCurr && isLeftLineActive ? `arrow-head-dg-${item.id}` : `arrow-head-base-dg-${item.id}`})`}
                  />
              ...
              return (
                <g key={`right-group-dg-${i}`}>
                  ...
                  <path
                    ...
                    markerEnd={`url(#${isCurr && isRightLineActive ? `arrow-head-dg-${item.id}` : `arrow-head-base-dg-${item.id}`})`}
                  />
                  {/* Animated Electric Pulse Flow on active line */}
            ...
            {paths.left[activeStep] && isLeftLineActive && !prefersReducedMotion && (
              <g>
                <animateMotion
                  key={`particle-left-dg-${activeStep}-${cycleKey}`}
                ...
                />
                {/* Outer halo */}
                <circle r="8.5" fill={currentItem.color} opacity="0.35" filter="url(#line-glow-dg)" />
                {/* Mid glow */}
            ...
            {paths.right[activeStep] && isRightLineActive && !prefersReducedMotion && (
              <g>
                <animateMotion
                  key={`particle-right-dg-${activeStep}-${cycleKey}`}
              ...
              >
                ...
                >
                  ...
                  <AnimatePresence mode="wait">
                    <motion.div
                      key={`center-transform-dg-${activeStep}-${phase}`}
