...
export default function WorkflowCanvas({
  ...
  const step01 = WORKFLOW_STEPS.find((s) => s.id === '01') // 01 ICP & TAM Definition
  const step02 = WORKFLOW_STEPS.find((s) => s.id === '02') // 02 Demand Strategy
  const step03 = WORKFLOW_STEPS.find((s) => s.id === '03') // 03 Account & Intent Mapping
  const step04 = WORKFLOW_STEPS.find((s) => s.id === '04') // 04 Content & Offer Creation
  const step05 = WORKFLOW_STEPS.find((s) => s.id === '05') // 05 Omnichannel Cadence
  const step06 = WORKFLOW_STEPS.find((s) => s.id === '06') // 06 Multi-Channel Launch
  const step07 = WORKFLOW_STEPS.find((s) => s.id === '07') // 07 Engagement & Telemetry
  const step08 = WORKFLOW_STEPS.find((s) => s.id === '08') // 08 BANT Lead Qualification
  const step09 = WORKFLOW_STEPS.find((s) => s.id === '09') // 09 Pipeline Acceleration

  return (
    <div className="relative w-full max-w-[1300px] mx-auto">
      {/* ── Flow Canvas (Seamless Background & Borderless) ── */}
      <div
      ...
      >
        ...
        {/* ── CARD 01: ICP & TAM Definition (Top Row, Left) ── */}
        <div
        ...
        {/* ── CARD 02: Demand Strategy (Top Row, Right) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '52.85%',
        ...
        {/* ── CARD 03: Account & Intent Mapping (Middle Row, Col 1) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '1.42%',
        ...
        {/* ── CARD 04: Content & Offer Creation (Middle Row, Col 2) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '18.57%',
        ...
        {/* ── CARD 05: Omnichannel Cadence (Middle Row, Col 3) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '35.71%',
        ...
        {/* ── CARD 06: Multi-Channel Launch (Middle Row, Col 4) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '52.85%',
        ...
        {/* ── CARD 07: Engagement & Telemetry (Middle Row, Col 5) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '70.00%',
        ...
        {/* ── FINAL TARGET: QUALIFIED B2B PIPELINE (Middle Row, Col 6) ── */}
        <div
        ...
        {/* ── CARD 08: BANT Lead Qualification (Bottom Row, Left of 09) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '37.85%',
        ...
        {/* ── CARD 09: Pipeline Acceleration (Bottom Row, Below 07) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '70.00%',
