...
export default function WorkflowCanvas({
  ...
  const step01 = WORKFLOW_STEPS.find((s) => s.id === '01') // 01 ICP & Audience
  const step02 = WORKFLOW_STEPS.find((s) => s.id === '02') // 02 Campaign Strategy
  const step03 = WORKFLOW_STEPS.find((s) => s.id === '03') // 03 Data & Segmentation
  const step04 = WORKFLOW_STEPS.find((s) => s.id === '04') // 04 Email Creation
  const step05 = WORKFLOW_STEPS.find((s) => s.id === '05') // 05 Personalization
  const step06 = WORKFLOW_STEPS.find((s) => s.id === '06') // 06 Campaign Launch
  const step07 = WORKFLOW_STEPS.find((s) => s.id === '07') // 07 Engagement Tracking
  const step08 = WORKFLOW_STEPS.find((s) => s.id === '08') // 08 Lead Qualification
  const step09 = WORKFLOW_STEPS.find((s) => s.id === '09') // 09 Optimize & Nurture
  ...
  return (
    <div className="relative w-full max-w-[1300px] mx-auto">
      ...
      >
        ...
        {/* ── CARD 01: ICP & Audience (Top Row, Left) ── */}
        <div
        ...
        {/* ── CARD 02: Campaign Strategy (Top Row, Right) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '52.85%',
        ...
        {/* ── CARD 03: Data & Segmentation (Middle Row, Col 1) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '1.42%',
        ...
        {/* ── CARD 04: Email Creation (Middle Row, Col 2) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '18.57%',
        ...
        {/* ── CARD 06: Campaign Launch (Middle Row, Col 4) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '52.85%',
        ...
        {/* ── CARD 07: Engagement Tracking (Middle Row, Col 5) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '70.00%',
        ...
        {/* ── CARD 09: Optimize & Nurture (Bottom Row, Below 07) ── */}
        <div
          className="absolute z-10"
          style={{
            left: '70.00%',
