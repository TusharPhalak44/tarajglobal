...
// ─── Command Center Flow Visual for Webinar Services ─────────────────────────

const FLOW_STAGES = [
  { id: 'target',    label: 'Target Audience',       sub: 'Ideal Customer Profile',      color: '#00A6FF', icon: 'target'    },
  { id: 'decision',  label: 'Decision Maker',        sub: 'Verified Senior Contact',     color: '#38BDF8', icon: 'person'    },
  { id: 'promote',   label: 'Webinar Promotion',     sub: 'Multi-Touch Outreach',       color: '#00A6FF', icon: 'video'     },
  { id: 'register',  label: 'Verified Registration', sub: 'Confirmed Attendee Seat',    color: '#FFA600', icon: 'check'     },
  { id: 'engage',    label: 'Live Engagement',       sub: 'Attended · Polls · Q&A',     color: '#72D669', icon: 'engage'    },
  { id: 'qualified', label: 'Qualified Pipeline',    sub: 'Sales Opportunity Ready',    color: '#FF6D00', icon: 'qualified' },
]

const STATUS_PILLS = [
  { label: 'Targeted',    dot: '#00A6FF' },
  { label: 'Registered',  dot: '#38BDF8' },
  { label: 'Attended',    dot: '#FFA600' },
  { label: 'Engaged',     dot: '#72D669' },
  { label: 'Qualified',   dot: '#FF6D00' },
]
...
function VideoIcon({ color }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <rect x="2" y="4" width="10" height="10" rx="2" stroke={color} strokeWidth="1.5" />
      <path d="M12 7.5L16 5V13L12 10.5V7.5Z" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
...
function CheckIcon({ color }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      ...
      <path d="M5.5 9.5L7.5 11.5L12.5 6.5" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
...
function StageIcon({ type, color }) {
  ...
  if (type === 'video')     return <VideoIcon color={color} />
...
function CommandCenter({ prefersReducedMotion }) {
  ...
  return (
    <div
      ...
      role="img"
      aria-label="Animated B2B webinar services campaign workflow from targeted audience to qualified pipeline"
    >
      {/* Grid background */}
      <div
      ...
      <div className="flex items-center gap-2 px-5 py-3.5 border-b border-white/[0.06]">
        ...
        <span className="ml-2 text-[10px] font-mono font-bold tracking-[0.2em] text-[#00A6FF]/80 uppercase">
          Webinar Campaign Command Center
        </span>
      ...
      <div className="border-t border-white/[0.05] px-5 py-3 grid grid-cols-3 gap-3">
        {[
          ...
          { label: 'Attendee Rate', value: 'Engaged', color: '#FFA600' },
          { label: 'Qualification', value: 'In Progress', color: '#00A6FF' },
        ].map((m) => (
...
const Hero = () => {
  ...
  const handleSeeHowItWorks = () => {
    const el = document.getElementById('what-is-b2b-webinar-services') || document.getElementById('webinar-process')
  ...
  return (
    <section
      ...
      aria-label="B2B Webinar Services Hero"
    >
      {/* Background */}
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-4 sm:pt-6 sm:pb-6 lg:pt-6 lg:pb-6">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="flex flex-col justify-center order-1 lg:order-1">
            ...
            >
              <span className="block pb-1 sm:pb-1.5">Turn Targeted Webinars</span>
              <span
            ...
            >
              Reach the right decision makers with targeted, high-impact B2B webinars designed to drive verified registrations, engage senior stakeholders, and convert live event participation into sales-qualified pipeline.
            </motion.p>
