...
// ─── Command Center Flow Visual for ABM ────────────────────────────────────────

const FLOW_STAGES = [
  { id: 'target',    label: 'Target Accounts',        sub: 'High-Value ICP Selection',      color: '#00A6FF', icon: 'target'    },
  { id: 'intel',     label: 'Account Intelligence',   sub: 'Tech Stack & Intent Signals',    color: '#38BDF8', icon: 'intel'     },
  { id: 'decision',  label: 'Stakeholder Mapping',    sub: 'Buying Committee Roles',        color: '#00A6FF', icon: 'person'    },
  { id: 'engage',    label: 'Personalized Touch',     sub: 'Multi-Channel Outreach',        color: '#FFA600', icon: 'engage'    },
  { id: 'intent',    label: 'Intent Tracking',        sub: 'Account Activity Signals',      color: '#72D669', icon: 'reply'     },
  { id: 'qualified', label: 'Pipeline Opportunity',   sub: 'Sales-Ready Enterprise Deal',   color: '#FF6D00', icon: 'qualified' },
]

const STATUS_PILLS = [
  { label: 'Identified',    dot: '#00A6FF' },
  { label: 'Researched',    dot: '#38BDF8' },
...
function StageIcon({ type, color }) {
  ...
  if (type === 'intel')     return <IntelIcon color={color} />
...
function CommandCenter({ prefersReducedMotion }) {
  ...
  return (
    <div
      ...
      role="img"
      aria-label="Animated Account-Based Marketing campaign workflow from targeted accounts to sales-ready enterprise pipeline"
    >
      {/* Grid background */}
      <div
      ...
      <div className="border-t border-white/[0.05] px-5 py-3 grid grid-cols-3 gap-3">
        {[
          { label: 'Target Account Focus', value: 'Tier-1 TAL', color: '#72D669' },
          { label: 'Buying Committee', value: 'Mapped', color: '#FFA600' },
          { label: 'Pipeline Velocity', value: 'Accelerated', color: '#00A6FF' },
        ].map((m) => (
...
const Hero = () => {
  ...
  const handleSeeHowItWorks = () => {
    const el = document.getElementById('abm-process')
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-4 sm:pt-6 sm:pb-6 lg:pt-6 lg:pb-6">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="flex flex-col justify-center order-1 lg:order-1">
            ...
            >
              Taraj Global helps B2B technology and SaaS companies identify high-value target accounts, engage key decision-makers, and create qualified opportunities through data-driven account-based marketing.
            </motion.p>
