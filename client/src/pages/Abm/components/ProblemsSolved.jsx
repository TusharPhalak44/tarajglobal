...
// ─── 6 Problem → Engine → Solution Transformation Pairs (ABM) ───────────────────────
...
const FLOW_ITEMS = [
  {
    ...
    num: '01',
    category: 'PRECISION',
    engineLabel: 'ICP Account Engine',
    engineAction: 'Target Qualified Accounts',
    problem: {
      title: 'Wasted Outreach',
      sub: 'Unqualified Leads & Broad Lists',
      icon: FilterX,
    },
    solution: {
      title: 'Precision Targeting',
      sub: 'High-Value Account Lists',
      icon: Focus,
    },
    color: '#00A6FF',
  ...
  },
  {
    id: 1,
    num: '02',
    category: 'EXECUTIVE REACH',
    engineLabel: 'Buyer Persona AI',
    engineAction: '1:1 Executive Personalize',
    problem: {
      title: 'Low Engagement',
      sub: 'Ignored Generic Outreach',
      icon: MailX,
    },
    solution: {
      title: 'Executive Messaging',
      sub: 'Role & Industry Tailored',
  ...
  },
  {
    ...
    num: '03',
    category: 'ALIGNMENT',
    engineLabel: 'Unified RevOps',
    engineAction: 'Align Sales & Marketing',
    problem: {
      title: 'Team Misalignment',
      sub: 'Siloed Sales & Marketing',
      icon: ShieldAlert,
    },
    solution: {
  ...
  },
  {
    ...
    engineAction: 'Multi-Touch Orchestration',
    problem: {
      title: 'Single-Touch Drops',
      sub: 'Ineffective Lead Nurturing',
      icon: Workflow,
    },
    solution: {
      title: 'Omnichannel Cadence',
      sub: 'Multi-Touch Coordinated Flows',
      icon: Send,
    },
    color: '#F59E0B',
  ...
  },
  {
    id: 4,
    num: '05',
    category: 'ACCOUNT TELEMETRY',
    engineLabel: 'Intent Telemetry',
    engineAction: 'Full-Funnel Attribution',
    problem: {
      title: 'Zero Visibility',
      sub: 'Blind To Account Intent',
      icon: EyeOff,
    },
    solution: {
      title: 'Account Intelligence',
      sub: 'Intent & Pipeline Attribution',
      icon: BarChart3,
    },
    color: '#EC4899',
  ...
  },
  {
    ...
    engineLabel: 'Velocity Engine',
    engineAction: 'Accelerate Pipeline Deals',
    problem: {
      title: 'Stalled Sales Cycles',
      sub: 'Long Buying Delays',
      icon: Clock,
    },
    solution: {
      title: 'Accelerated Velocity',
      sub: 'High-Value Buying Touchpoints',
      icon: Zap,
    },
    color: '#06B6D4',
...
const ProblemsSolved = () => {
  ...
  const updatePaths = useCallback(() => {
    ...
    // Calculate Y offsets on circle rim for 6 distinct ports
    const angleOffsets = [-0.45, -0.27, -0.09, 0.09, 0.27, 0.45]
    ...
    FLOW_ITEMS.forEach((_, idx) => {
      ...
      if (pEl) {
        ...
        const rimY = centerY + Math.sin(angle) * (radius * 0.72)
  ...
  return (
    <section
      ...
      aria-label="What Problems Does Account-Based Marketing Solve"
    >
      ...
      <div className="relative max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-4 sm:mb-5">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black tracking-tight leading-tight uppercase">
            <span>What Problems Does Account-Based </span>
          ...
          <p className="text-sm sm:text-base text-slate-600 dark:text-slate-400 leading-relaxed max-w-xl mx-auto mt-1.5 font-normal">
            From wasted outreach to fragmented sales alignment, we transform high-friction obstacles into predictable enterprise pipeline.
          </p>
        ...
        <div
          ref={containerRef}
          className="hidden md:block relative min-h-[420px] lg:min-h-[440px] w-full"
        >
          ...
          >
            <defs>
              <filter id="abm-line-glow" x="-20%" y="-20%" width="140%" height="140%">
              ...
              {FLOW_ITEMS.map((item) => (
                <React.Fragment key={`arrow-${item.id}`}>
                  ...
                  <marker
                    id={`abm-arrow-head-base-${item.id}`}
                    viewBox="0 0 10 10"
                    refX="7"
                    refY="5"
                    markerWidth="5"
              ...
              return (
                <g key={`left-group-${i}`}>
                  ...
                  <path
                    ...
                    markerEnd={`url(#${isCurr && isLeftLineActive ? `abm-arrow-head-${item.id}` : `abm-arrow-head-base-${item.id}`})`}
                  />
              ...
              return (
                <g key={`right-group-${i}`}>
                  ...
                  <path
                    ...
                    markerEnd={`url(#${isCurr && isRightLineActive ? `abm-arrow-head-${item.id}` : `abm-arrow-head-base-${item.id}`})`}
                  />
                  {/* Animated Electric Pulse Flow on active line */}
            ...
            {paths.left[activeStep] && isLeftLineActive && !prefersReducedMotion && (
              <g>
                <animateMotion
                  ...
                  fill="freeze"
                />
                <circle r="8.5" fill={currentItem.color} opacity="0.35" filter="url(#abm-line-glow)" />
          ...
          <div className="relative z-10 grid grid-cols-12 gap-2.5 lg:gap-3 items-center">

            {/* ── LEFT: 6 Compact Horizontal Problem Cards (4 cols) ── */}
            <div className="col-span-4 flex flex-col justify-between gap-1.5 lg:gap-2">
              {FLOW_ITEMS.map((item, idx) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => handleSelectStep(idx)}
                    className={`
                      relative flex items-center gap-2 p-1.5 lg:p-2 rounded-xl transition-all duration-300 cursor-pointer select-none border
                      ${isActive
                        ? 'bg-white dark:bg-[#0B1527] border-primary shadow-xl shadow-primary/25 scale-[1.05] -translate-x-1 ring-2 ring-primary/50 z-30'
                  ...
                  >
                    {/* Square Icon Badge */}
                    <div
                      className={`w-7 h-7 rounded-lg flex items-center justify-center text-white shrink-0 shadow-xs transition-transform duration-300 ${isActive ? 'scale-110 shadow-md' : ''}`}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-[12px] lg:text-[12.5px] font-bold text-slate-900 dark:text-white leading-tight truncate flex items-center justify-between">
                      ...
                      <div className="text-[9.5px] lg:text-[10px] text-slate-500 dark:text-slate-400 leading-tight mt-0.5 truncate">
                        {item.problem.sub}
                      </div>
                    </div>
                  </div>
                ...
                >
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      <div className="text-[10.5px] lg:text-[11px] font-black uppercase tracking-tight text-white leading-tight text-center px-1">
                        {currentItem.engineAction}
                      </div>

                      {/* Status indicator */}
                      <div className="text-[8px] font-mono tracking-wider mt-0.5 text-center">
                        {phase === 'centerReact' ? (
            ...
            {/* ── RIGHT: 6 Compact Horizontal Solution Cards (4 cols) ── */}
            <div className="col-span-4 flex flex-col justify-between gap-1.5 lg:gap-2">
              {FLOW_ITEMS.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Text block */}
                    <div className="min-w-0 flex-1">
                      <div className="text-[12px] lg:text-[12.5px] font-bold text-slate-900 dark:text-white leading-tight truncate">
                        {item.solution.title}
                      </div>
                      <div className="text-[9.5px] lg:text-[10px] text-slate-500 dark:text-slate-400 leading-tight mt-0.5 truncate">
                        {item.solution.sub}
