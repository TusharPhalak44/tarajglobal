...
// ─── 5 Problem → Engine → Solution Transformation Pairs (MQL Focused) ─────────
...
const FLOW_ITEMS = [
  {
    ...
    engineLabel: 'ICP Precision Engine',
    engineAction: 'Firmographic Filtering',
    problem: {
      title: 'Untargeted Lead Lists',
      sub: 'Non-Fit Accounts & Low Match',
      icon: FilterX,
    },
    solution: {
      title: 'ICP Account Precision',
      sub: 'Revenue & Headcount Verified',
      icon: Focus,
    },
    color: '#00A6FF', // Blue
  ...
  },
  {
    ...
    engineLabel: 'Intent Surge Engine',
    engineAction: 'Behavioral Signal Scoring',
    problem: {
      title: 'Passive Form Fills',
      sub: 'Zero Intent & Unresponsive Contacts',
      icon: MailX,
    },
    solution: {
      title: 'Verified Buying Signals',
      sub: 'Active Solution Researchers',
  ...
  },
  {
    ...
    engineLabel: 'Triple-Layer Verifier',
    engineAction: 'Zero Bounce Validation',
    problem: {
      title: 'High Bounce Rates',
      sub: 'Stale Contacts & Dead Inboxes',
      icon: ShieldAlert,
    },
    solution: {
  ...
  },
  {
    ...
    engineLabel: 'Committee Mapping Core',
    engineAction: 'Seniority & Role Vetting',
    problem: {
      title: 'Gatekeeper Dead-Ends',
      sub: 'Non-Buying Influencer Leads',
      icon: Workflow,
    },
    solution: {
      title: 'Budget-Holder Access',
      sub: 'Vetted VP & C-Level Authority',
      icon: Send,
    },
    color: '#F59E0B', // Amber
  ...
  },
  {
    ...
    engineLabel: 'CRM Telemetry Sync',
    engineAction: 'Sales-Ready MQL Handoff',
    problem: {
      title: 'Pipeline Disconnect',
      sub: 'Unqualified Leads Stalling SDRs',
      icon: EyeOff,
    },
    solution: {
      title: 'Accelerated SDR Conversion',
      sub: 'Enriched Intent & CRM Ready',
      icon: BarChart3,
    },
    color: '#EC4899', // Pink
...
const ProblemsSolved = () => {
  ...
  return (
    <section
      id="what-problems-mql-services-solve"
      ref={sectionRef}
      ...
      aria-label="What Problems Do MQL Services Solve"
    >
      ...
      <div className="relative max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-4 sm:mb-5">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black tracking-tight leading-tight uppercase">
            ...
            <span className="bg-gradient-to-r from-primary via-[#00c8ff] to-cta bg-clip-text text-transparent">
              MQL Lead Generation Solve?
            </span>
          </h2>

          <p className="text-sm sm:text-base text-slate-600 dark:text-slate-400 leading-relaxed max-w-xl mx-auto mt-1.5 font-normal">
            From unqualified form fills to stale contact data, we transform pipeline bottlenecks into high-intent, sales-ready Marketing Qualified Leads.
          </p>
        ...
        >
          ...
          >
            <defs>
              <filter id="line-glow-mql" x="-20%" y="-20%" width="140%" height="140%">
              ...
              {FLOW_ITEMS.map((item) => (
                <React.Fragment key={`arrow-${item.id}`}>
                  ...
                  <marker
                    id={`arrow-head-base-mql-${item.id}`}
                    viewBox="0 0 10 10"
                    refX="7"
                    refY="5"
                    markerWidth="5"
              ...
              return (
                <g key={`left-group-${i}`}>
                  ...
                  <path
                    ...
                    markerEnd={`url(#${isCurr && isLeftLineActive ? `arrow-head-mql-${item.id}` : `arrow-head-base-mql-${item.id}`})`}
                  />
              ...
              return (
                <g key={`right-group-${i}`}>
                  ...
                  <path
                    ...
                    markerEnd={`url(#${isCurr && isRightLineActive ? `arrow-head-mql-${item.id}` : `arrow-head-base-mql-${item.id}`})`}
                  />
                  {/* Animated Electric Pulse Flow on active line */}
            ...
            {paths.left[activeStep] && isLeftLineActive && !prefersReducedMotion && (
              <g>
                ...
                />
                {/* Outer halo */}
                <circle r="8.5" fill={currentItem.color} opacity="0.35" filter="url(#line-glow-mql)" />
                {/* Mid glow */}
                    ...
                    >
                      ...
                      <div className="text-[8.5px] font-mono tracking-wider mt-0.5 text-center">
                        {phase === 'centerReact' ? (
                          ...
                          <span className="text-emerald-300 font-bold">✓ QUALIFIED</span>
                        ) : (
                          <span className="text-sky-300/90">● SCORING</span>
                        )}
                      </div>
                    </motion.div>
                  </AnimatePresence>
