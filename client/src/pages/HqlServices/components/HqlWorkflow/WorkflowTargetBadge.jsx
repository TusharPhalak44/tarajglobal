...
import { Target, PartyPopper, Award } from 'lucide-react'
...
const CONFETTI_PARTICLES = Array.from({ length: 30 }).map((_, i) => {
  ...
  const dist = 70 + (i % 5) * 22 // 70px to 160px burst radius
...
export default function WorkflowTargetBadge({
  ...
  // Trigger celebration automatically whenever process reaches the end (isActive becomes true)
  useEffect(() => {
    if (isActive && !prevActiveRef.current) {
      setBurstKey((k) => k + 1)
      const msg = CELEBRATION_MESSAGES[Math.floor(Math.random() * CELEBRATION_MESSAGES.length)]
      const newId = Date.now() + Math.random()
      setFloatingTexts((prev) => [...prev.slice(-1), { id: newId, text: msg }])
  ...
  return (
    <div
      ...
      title="Click to celebrate Qualified HQL Leads!"
    >
      {/* ── Outer Soft Atmospheric Glow (No Hard Edges) ── */}
      <div
      ...
      {/* ── RADIAL LUMINOUS FLASH WAVE (Triggers on Active End & Click) ── */}
      <AnimatePresence>
        {burstKey > 0 && !isReducedMotion && (
          <motion.div
            key={`luminous-flash-${burstKey}`}
      ...
      {/* ── CELEBRATION BLAST: 360-Degree Confetti & Sparkles Explosion on End & Click ── */}
      <AnimatePresence>
        {burstKey > 0 && !isReducedMotion && (
          <motion.div
            key={`confetti-group-${burstKey}`}
            initial={{ opacity: 1 }}
            animate={{ opacity: 0 }}
            exit={{ opacity: 0 }}
      ...
      {/* ── FLOATING SUCCESS CHIPS ON ACTIVE & CLICK ── */}
      ...
      {/* ── Main Circular Core Container (Completely Borderless, Exact Email Marketing Match) ── */}
      <motion.div
        key={`badge-core-${burstKey}`}
        animate={
          isReducedMotion
            ? {}
            : burstKey > 0
        ...
        `}
        style={{
          background: isCelebrated || burstKey > 0
      ...
      >
        {/* ── Center Icon: Bullseye Target or Celebration Trophy ── */}
        ...
        {/* ── Celebration Sub-pill Badge (Borderless) ── */}
        <motion.div
          animate={burstKey > 0 ? { scale: [1, 1.15, 1] } : {}}
